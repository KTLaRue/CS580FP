...
function PerspectiveCamera(eye::Vec3, view::Vec3, up::Vec3, focal::Real, canv_height::Int, canv_width::Int)
    ...
    # Your implementation:
    #
    # calculate our u, v, w values, normalizing each
    w = normalize(-view)
...
function pixel_to_ray(camera::PerspectiveCamera, i, j) 
    ...
    # get u, v coordinates from i, j
    u = ((j - 0.5)/camera.canv_width) - 0.5
    v = -(((i - 0.5)/camera.canv_height) - 0.5)

    # calculate ray from eye to a given pixel_ij
    p = camera.eye
    d = camera.u_axis * u + camera.v_axis * v - camera.focal * camera.w_axis
    return Ray(p, d)
    ###############
    # END TODO 8b #
    ###############
